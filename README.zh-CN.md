...
- [`note-setup`](./note/note-setup.tex)：默认样式，采用彩色边框和浅色背景。
- [`note-setup-attached`](./note/note-setup-attached.tex)：由 `note-setup` 派生，采用附着式标题。
- [`note-setup-leftbar`](./note/note-setup-leftbar.tex)：采用彩色左边线和浅色背景。
- [`note-setup-shaded`](./note/note-setup-shaded.tex)：采用无边框的浅色背景。
- [`note-setup-plain`](./note/note-setup-plain.tex)：采用标准定理样式，不依赖 tcolorbox。
- [`note-setup-dark`](./note/note-setup-dark.tex)：采用深色页面和定理框，属于实验性样式。
...
- [`beamer-setup`](./beamer/beamer-setup.tex)：默认样式，采用顶部导航栏和圆角标题页。
- [`beamer-setup-minimal`](./beamer/beamer-setup-minimal.tex)：移除导航栏，并采用更加简洁的标题页。
- [`beamer-setup-console`](./beamer/beamer-setup-console.tex)：采用深色终端风格，灵感来自 [kmbeamer](https://github.com/kmaed/kmbeamer)。
